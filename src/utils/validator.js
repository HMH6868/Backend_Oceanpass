...
export function assertUpdateProfile(body) {
  const { name, phone, email, password, role, avatar } = body;

  if (email !== undefined || password !== undefined || role !== undefined) {
    const err = new Error('Chỉ được cập nhật name, phone và avatar');
    err.status = 400;
    throw err;
  }

  if (name === undefined && phone === undefined && avatar === undefined) {
    const err = new Error('Cần cung cấp ít nhất name, phone hoặc avatar');
    err.status = 400;
    throw err;
  }
  ...
  if (avatar !== undefined) {
    if (typeof avatar !== 'string' || avatar.trim().length === 0) {
      const err = new Error('avatar phải là một chuỗi không rỗng');
      err.status = 400;
      throw err;
    }
    // Tùy chọn: Thêm kiểm tra định dạng URL
    try {
      new URL(avatar);
    } catch (_) {
      const err = new Error('avatar phải là một URL hợp lệ');
      err.status = 400;
      throw err;
    }
  }
...
export function assertCreatePromotion(body) {
  ...
  if (from && isNaN(from.getTime())) {
    const err = new Error('valid_from không hợp lệ');
    err.status = 400;
    throw err;
  }
  if (to && isNaN(to.getTime())) {
    const err = new Error('valid_to không hợp lệ');
    err.status = 400;
    throw err;
  }
  if (from && to && from > to) {
...
export function assertUpdatePromotion(body) {
  if (!body || typeof body !== 'object') {
    const err = new Error('Body không hợp lệ');
    err.status = 400;
    throw err;
  }
  ...
  for (const k of keys) {
    if (!allowed.includes(k)) {
      const err = new Error(`Trường không được phép: ${k}`);
      err.status = 400;
      throw err;
    }
  }
  ...
  if (body.code !== undefined) {
    if (
      ...
      body.code.trim().length > 64
    ) {
      const err = new Error('code phải là chuỗi 3–64 ký tự');
      err.status = 400;
      throw err;
    }
  }
  if (body.name !== undefined) {
    if (
      ...
      const err = new Error('name phải là chuỗi 3–200 ký tự');
      err.status = 400;
      throw err;
    }
  }
  ...
  if (body.min_amount !== undefined) {
    ...
    if (!Number.isFinite(v) || v < 0) {
      const err = new Error('min_amount phải là số ≥ 0');
      err.status = 400;
      throw err;
    }
  }
  if (body.max_discount !== undefined) {
    ...
    if (!Number.isFinite(v) || v < 0) {
      const err = new Error('max_discount phải là số ≥ 0');
      err.status = 400;
      throw err;
    }
  }
  ...
  if (body.valid_from && body.valid_to) {
    const from = new Date(body.valid_from);
    const to = new Date(body.valid_to);
    if (from > to) {
      const err = new Error('valid_from phải ≤ valid_to');
      err.status = 400;
      throw err;
...
export function assertCreatePort(body) {
  ...
  if (!name || typeof name !== 'string' || name.trim().length < 2) {
    const e = new Error('name bắt buộc (>=2 ký tự)');
    e.status = 400;
    throw e;
  }
  if (!code || typeof code !== 'string' || code.trim().length < 2 || code.trim().length > 10) {
    const e = new Error('code bắt buộc (2–10 ký tự)');
    e.status = 400;
    throw e;
  }
  if (!city || typeof city !== 'string' || city.trim().length < 2) {
    const e = new Error('city bắt buộc');
    e.status = 400;
    throw e;
  }
  if (latitude === undefined || longitude === undefined) {
    const e = new Error('latitude và longitude bắt buộc');
    e.status = 400;
    throw e;
  }
  ...
  if (!Number.isFinite(lat) || lat < -90 || lat > 90) {
    const e = new Error('latitude không hợp lệ');
    e.status = 400;
    throw e;
  }
  if (!Number.isFinite(lon) || lon < -180 || lon > 180) {
    const e = new Error('longitude không hợp lệ');
    e.status = 400;
    throw e;
  }
  if (
    id !== undefined &&
    (typeof id !== 'string' || id.trim().length < 2 || id.trim().length > 50)
  ) {
    const e = new Error('id (nếu có) phải là chuỗi 2–50 ký tự');
    e.status = 400;
    throw e;
  }
  if (address !== undefined && typeof address !== 'string') {
    const e = new Error('address phải là chuỗi');
    e.status = 400;
    throw e;
  }
}
...
export function assertUpdatePort(body) {
  if (!body || typeof body !== 'object') {
    const e = new Error('Body không hợp lệ');
    e.status = 400;
    throw e;
  }
  ...
  const keys = Object.keys(body);
  if (keys.length === 0) {
    const e = new Error('Không có trường nào để cập nhật');
    e.status = 400;
    throw e;
  }
  for (const k of keys)
    if (!allowed.includes(k)) {
      const e = new Error(`Trường không được phép: ${k}`);
      e.status = 400;
      throw e;
    }

  if (body.name !== undefined && (typeof body.name !== 'string' || body.name.trim().length < 2)) {
    const e = new Error('name không hợp lệ');
    e.status = 400;
    throw e;
  }
  if (
    body.code !== undefined &&
    (typeof body.code !== 'string' || body.code.trim().length < 2 || body.code.trim().length > 10)
  ) {
    const e = new Error('code không hợp lệ');
    e.status = 400;
    throw e;
  }
  if (body.city !== undefined && (typeof body.city !== 'string' || body.city.trim().length < 2)) {
    const e = new Error('city không hợp lệ');
    e.status = 400;
    throw e;
  }
...
export function assertCreateRoute(body) {
  ...
  if (!from_port_id || !to_port_id) {
    const e = new Error('from_port_id và to_port_id bắt buộc');
    e.status = 400;
    throw e;
  }
  if (from_port_id === to_port_id) {
    const e = new Error('from_port_id và to_port_id không được trùng');
    e.status = 400;
    throw e;
  }

  if (distance_km !== undefined && distance_km !== null) {
    const v = Number(distance_km);
    if (!Number.isFinite(v) || v < 0) {
      const e = new Error('distance_km phải ≥ 0');
      e.status = 400;
      throw e;
    }
  }
  if (estimated_duration_minutes !== undefined && estimated_duration_minutes !== null) {
    ...
    if (!Number.isInteger(v) || v < 0) {
      const e = new Error('estimated_duration_minutes phải là số nguyên ≥ 0');
      e.status = 400;
      throw e;
    }
  }
...
export function assertUpdateRoute(body) {
  ...
  if (body.distance_km !== undefined && body.distance_km !== null) {
    const v = Number(body.distance_km);
    if (!Number.isFinite(v) || v < 0) {
      const e = new Error('distance_km phải ≥ 0');
      e.status = 400;
      throw e;
